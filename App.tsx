...
import { ReadingDataResponse, DailyData, CumulativeData, ProcessedYearlyData, MonthlyData, ProcessedData, AchievementCategory, PersonalInsightsData, Achievement, AchievementTier } from './types';
...
import { BookIcon, CalendarIcon, TargetIcon, WordIcon } from './components/icons';
import AffiliateBanner from './components/AdsenseComponent';
...
const App: React.FC = () => {
    ...
    try {
        ...
        if (!cachedData) {
            ...
            const [currentMonthData, prevMonthData] = await Promise.all([
                hamelnApiService.getMonthReadingData(token, currentYear, currentMonth),
            ...
            if (prevMonthData.data[prevMonthYear] && prevMonthData.data[prevMonthYear][prevMonth]) {
                if (!updatedData.data[prevMonthYear]) updatedData.data[prevMonthYear] = {};
                updatedData.data[prevMonthYear][prevMonth] = prevMonthData.data[prevMonthYear][prevMonth];
            }
  ...
  const processedData: ProcessedData | null = useMemo(() => {
        ...
        const yearlyData = Object.values(dataByYear).sort((a, b) => b.year - a.year);
        ...
        if (sortedDates.length > 0) {
            ...
            const diffFromCutoff = (dataCutoffDate.getTime() - lastReadingDate.getTime()) / (1000 * 60 * 60 * 24);
        ...
        const maxMonthlyWords = Math.max(0, ...sortedAllMonthlyData.map(m => m.word_count));
        ...
        const achievementsByCategory: AchievementCategory[] = ACHIEVEMENT_DEFINITIONS.map(cat => {
        ...
        const tierOrder: AchievementTier[] = ['bronze', 'silver', 'gold', 'platinum', 'diamond', 'special'];

        achievementsByCategory.forEach(category => {
            const unlocked = category.achievements.filter(ach => category.currentValue >= ach.value);
            if (unlocked.length > 0) {
                const latest = unlocked[unlocked.length - 1];
                if (!mostProudAchievement || tierOrder.indexOf(latest.tier) > tierOrder.indexOf(mostProudAchievement.tier)) {
                    mostProudAchievement = latest;
                }
            }
        });

        return { allMonthlyData: sortedAllMonthlyData, yearlyData, cumulativeData: cumulative, calendarData, levelData, achievementsByCategory, longestStreak, currentStreak, mostProudAchievement };
    }, [readingData]);
  ...
  const renderCurrentView = () => {
    ...
    switch (currentView) {
        ...
        case 'achievements':
            return <AchievementsPage achievementsByCategory={processedData.achievementsByCategory} />;
        case 'dashboard':
        default:
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex flex-col min-h-screen aurora-background">
          ...
          <main className="flex-grow">
            <LandingPage onStart={() => setView('app')} onOpenModal={setModalContent} />
          </main>
        </div>
      );
    }
    ...
    if (!token) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center aurora-background p-4">
                 <Login onLoginSuccess={handleLoginSuccess} onOpenModal={setModalContent} />
            </div>
        );
    }
    ...
    return (
      <div className="flex h-screen aurora-background font-sans">
        ...
        <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} onLogout={handleLogout} currentView={currentView} />
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            <main className="flex-grow container mx-auto p-4 md:p-6">
              ...
              {error && <ErrorMessage message={error} onRetry={fetchReadingData} />}
              {processedData && (
                <>
                  {renderCurrentView()}
                  ...
                  {/*
                    アフィリエイトバナー
                    AdSenseの代替として、ログイン後のコンテンツページに表示します。
                  */}
                  <AffiliateBanner />
                </>
              )}
            ...
            <footer className="w-full bg-transparent py-4 mt-auto">
                <div className="container mx-auto text-center text-gray-500 text-sm">
                    ...
                    <div className="mt-2 space-x-4">
                        <button onClick={() => setModalContent('terms')} className="hover:text-primary transition-colors">利用規約</button>
                        <span>|</span>
                        <button onClick={() => setModalContent('privacy')} className="hover:text-primary transition-colors">プライバシーポリシー</button>
                         <span>|</span>
