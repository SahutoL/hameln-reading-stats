...
import { ReadingDataResponse, DailyData, CumulativeData, ProcessedYearlyData, MonthlyData, ProcessedData } from './types';
...
// 3 days ago filter
const getFilteredData = (data: ReadingDataResponse | null): ReadingDataResponse | null => {
    if (!data) return null;
...
const App: React.FC = () => {
  ...
  const processedData: ProcessedData | null = useMemo(() => {
        ...
        for (const yearStr in filteredReadingData.data) {
            ...
            for (const monthStr in yearData) {
                ...
                for (const dayStr in monthStats.daily_data) {
                    ...
                    calendarData.set(dateStr, monthStats.daily_data[dayStr].daily_word_count);
                }
            }
        }
        ...
        return { allMonthlyData: sortedAllMonthlyData, yearlyData, cumulativeData: cumulative, calendarData };
    }, [filteredReadingData]);

  const renderContent = () => {
    if (!token) {
      return <Login onLoginSuccess={handleLoginSuccess} onOpenModal={setModalContent} />;
    }
