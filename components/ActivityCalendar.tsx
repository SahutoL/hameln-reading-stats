    ...
    <div className="flex flex-col items-center">
      <div className="grid grid-rows-7 grid-flow-col gap-1 w-full overflow-x-auto p-2">
         {dates.map((date, index) => {
          ...
          const tooltipVPosition = dayOfWeek < 3 ? 'top-full mt-2' : 'bottom-full mb-2';
          const tooltipHPosition = 'left-1/2 -translate-x-1/2';
          
          return (
            <div key={index} className="relative group w-4 h-4 rounded-sm" style={{gridRow: dayOfWeek + 1}}>
              <div
                className={`w-full h-full ${getColor(wordCount)}`}
              />
              <div className={`absolute z-10 ${tooltipVPosition} ${tooltipHPosition} w-max p-2 text-xs text-white bg-gray-900 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`}>
                {wordCount > 0 ? `${wordCount.toLocaleString()} words` : 'No activity'} on {date.toLocaleDateString()}
              </div>
            </div>
          );
        })}
      </div>
       <div className="flex justify-between w-full text-xs text-gray-400 mt-2 px-2" style={{maxWidth: 'calc(53 * (1rem / 4 + 0.25rem))'}}>
