...
import { BadgeCheckIcon, TrophyIcon, ShieldCheckIcon } from "./icons";
...
const getTierColor = (tier: AchievementTier): string => {
  switch (tier) {
    ...
    case "platinum":
      return "#E5E4E2";
    case "diamond":
      return "#81e6d9"; // A bit different from pure cyan for visibility
    case "special":
...
const AchievementItem: React.FC<AchievementItemProps> = ({
  ...
  <div
    className={`flex items-center p-3 rounded-lg transition-all duration-300 ${
      unlocked ? "bg-surface" : "bg-surface/50"
    }`}
  >
    <ShieldCheckIcon
      tierColor={unlocked ? getTierColor(achievement.tier) : "#4A5568"}
      className="w-8 h-8 mr-4 flex-shrink-0"
    />
    <div>
      <h4
        className={`font-semibold ${
          unlocked ? "text-on-surface" : "text-gray-500"
        }`}
      >
        {achievement.name}
      </h4>
...
const AchievementCategoryCard: React.FC<{
  ...
  return (
    <div
      className={`bg-surface p-4 rounded-xl border border-gray-700/50 flex flex-col gap-3 transition-all duration-300 hover:border-primary hover:-translate-y-1 cursor-pointer ${
        isComplete ? "border-yellow-400/50" : ""
      }`}
      ...
      <div className="flex-grow space-y-2">
        {latestAchievement && (
          <div className="flex items-center gap-2 text-xs text-gray-400">
            <ShieldCheckIcon
              tierColor={getTierColor(latestAchievement.tier)}
          ...
          <>
            <div className="text-xs text-gray-400 mb-1 flex justify-between">
              ...
              <span>
                ...
                {nextAchievement.achievement!.value.toLocaleString()}
              </span>
            </div>
        ...
        {selectedCategory && (
          <div className="space-y-2">
            <p className="text-sm text-gray-400 pb-2 mb-2 border-b border-gray-700">
              達成状況: {selectedCategory.unlockedCount} /{" "}
