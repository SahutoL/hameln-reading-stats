...
class HamelnApiService {
  ...
  async getReadingData(token: string, yearFrom: number = 2024, yearTo?: number): Promise<ReadingDataResponse> {
    ...
    const response = await fetch(`${this.baseUrl}/api/v3/reading-data?${params.toString()}`, {
      headers: {
  ...
  async getMonthReadingData(token: string, year: number, month: number): Promise<ReadingDataResponse> {
    const params = new URLSearchParams();
    params.append('year', year.toString());
    params.append('month', month.toString());

    const response = await fetch(`${this.baseUrl}/api/v3/month-reading-data/basic?${params.toString()}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
